...
public class Charmander extends Pokemon {
    ...
    //constructor for the charmander class
    public Charmander(int x, int y)
    {
        ...
        name = "Charmander"; //sets its name.
        HP = 100; //sets its initial HP
        type = "Fire"; //sets its type
        //sets the moves the Pokemon Knows
        attacks[0] = "Flamethrower";
        attacks[1] = "Tackle";
        loadSprite("charmander.jpg"); //the correct image to load
    }
    ...
    public void attack(Pokemon p, int choice) {
        ...
        else
        {
            ...
            if(choice == 1)
            {
                //if the first attack was selected will use flamethrower
                //check for resistance or strength
                if(p.getType() == "Grass")
                {
                    //if it was a grass type it takes double damage
                    p.setHP(p.getHP() - 30);
                    powerPoints = powerPoints - 1; //they lost a use of their special attack
                    turnSummary = name + " used Flamethrower. It was super effective! " + p.getName() + " took 30 damage."; //a summary of the turn
                }
                else if(p.getType() == "Water")
                {
                    //if it was a water type it takes half damage
                    p.setHP(p.getHP() - 7.5);
                    powerPoints = powerPoints - 1; //they lost a use of their special attack
                    turnSummary = name + " used Flamethrower. Not very effective... " + p.getName() + " took 7.5 damage."; //a summary of the turn
                }
                else
                {
                    ...
                    p.setHP(p.getHP() - 15);
                    powerPoints = powerPoints - 1; //they lost a use of their special attack
                    turnSummary = name + " used Flamethrower. " + p.getName() + " took 15 damage."; //a summary of the turn
                }
            }
    ...
    public String speak() {
        ...
        String summary = "Name: " + name + " HP: " + HP + " Attacks: Flamethrower " + powerPoints + " Power Points, Tackle";
